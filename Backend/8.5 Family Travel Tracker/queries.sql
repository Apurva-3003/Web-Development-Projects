...
CREATE TABLE homework_submission (
  id SERIAL PRIMARY KEY,
  mark INTEGER,
  student_id INTEGER REFERENCES student(id) --Just missing the unique keyword, making it many to one--
);

-- Data --
INSERT INTO homework_submission (mark, student_id)
VALUES (98, 1), (87, 1), (88, 1)  --Inserting multiple data rows in 1 line --
...
-- Many to Many --
-- These type of relationships are made by simply taking multiple tables and making many to one relations of each of them with a new table --
-- class and student have many to many relation to each other, so we make enrollment tabel to connect them --
CREATE TABLE class (
  id SERIAL PRIMARY KEY,
  title VARCHAR(45)
);
...
--Many to One Relationship where many rows in this table can be mapped to one user from users table, using just user_id--
